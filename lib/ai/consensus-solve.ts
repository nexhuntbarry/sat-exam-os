...
async function solveOnce(q: QRow, variant: number): Promise<string | null> {
  ...
  try {
    const r = await generateObject({
      ...
      system: SOLVE_SYS,
      maxRetries: 2,
      messages: [{ role: "user", content: [{ type: "text", text: `${framings[variant % framings.length]}\n\nQuestion ${q.original_question_number}:\n${q.question_text}\n\nChoices:\n${ch}` }] }],
    });
...
async function verify(q: QRow, answer: string): Promise<{ ok: boolean; explanation: string } | null> {
  const ch = Array.isArray(q.choices)
    ? (q.choices as unknown[]).map((c) => (typeof c === "string" ? c : `${(c as { label?: string }).label}) ${(c as { text?: string }).text}`)).join("\n")
    : "";
  try {
...
export async function consensusResolve(
  moduleId: string,
  db: DbClient,
): Promise<{ attempted: number; settled: number; unresolved: number }> {
  const { data } = await db
    .from("questions")
    .select("id, original_question_number, question_text, choices, question_type, parsing_notes, official_answer, section")
  ...
  for (const q of targets) {
    const [a1, a2, a3] = await Promise.all([solveOnce(q, 0), solveOnce(q, 1), solveOnce(q, 2)]);
  ...
  console.log(`[consensus-resolve] module=${moduleId} attempted=${targets.length} settled=${settled} unresolved=${unresolved}`);
  return { attempted: targets.length, settled, unresolved };
}
