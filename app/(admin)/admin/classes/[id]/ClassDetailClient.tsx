  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">{classGroup.name}</h1>
      ...
      {nonMembers.length > 0 && (
        <div className="bg-white/3 border border-white/8 rounded-2xl p-5">
          <h2 className="text-soft-gray font-medium text-sm mb-3">Add Student</h2>
          <div className="flex gap-3">
            <select
              className="flex-1 bg-deep-navy border border-white/15 text-white rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-electric-blue/50 transition-colors"
              value={addStudentId}
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-electric-blue hover:bg-electric-blue/90 text-white font-medium text-sm disabled:opacity-60 transition-colors"
            >
              <UserPlus size={15} />
              Add
            </button>
      ...
      <div className="bg-white/3 border border-white/8 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/8">
          <h2 className="font-semibold text-soft-gray">Members</h2>
        </div>
        {members.length === 0 ? (
          <div className="py-12 text-center text-soft-gray/40 text-sm">
            No members yet. Add students above.
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="text-soft-gray/50 border-b border-white/8">
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      ...
                      <div className="text-soft-gray/50 text-xs">{u?.email}</div>
                    </td>
                    <td className="px-5 py-3 text-soft-gray/70">
                      {profile?.grade ? `Grade ${profile.grade}` : "—"}
                    </td>
                    <td className="px-5 py-3 text-soft-gray/70">{profile?.school ?? "—"}</td>
                    <td className="px-5 py-3 text-right">
                      <button
                        onClick={() => handleAction(m.student_id, "remove")}
                        disabled={loading}
                        className="flex items-center gap-1.5 ml-auto px-3 py-1.5 rounded-lg bg-rose/10 hover:bg-rose/20 text-rose text-xs font-medium transition-colors disabled:opacity-50"
                      >
                        <UserMinus size={13} />
                        Remove
                      </button>
